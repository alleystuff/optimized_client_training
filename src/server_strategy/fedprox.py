...
def fedprox_strategy(model, client_config, dataset_name, strategy_name, meta_action_type, proximal_mu, seed=42):
    """
    Implement FedProx Strategy
    """
    initial_parameters = fl.common.ndarrays_to_parameters(get_parameters(model)) #set intial parameters
    class SaveFedProxModelStrategy(fl.server.strategy.FedProx):
        def aggregate_fit(
            self,
            server_round: int,
    ...
    strategy = SaveFedProxModelStrategy(
        fraction_fit = 0.8,
