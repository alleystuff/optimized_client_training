...
def fedmedian_strategy(model, client_config, dataset_name, strategy_name, meta_action_type, seed=42):
    """
    Implement FedMedian Strategy
    """
    initial_parameters = fl.common.ndarrays_to_parameters(get_parameters(model)) #set intial parameters
    class SaveFedMedianModelStrategy(fl.server.strategy.FedMedian):
        def aggregate_fit(
            self,
            server_round: int,
            ...
            """Aggregate model weights using median and store checkpoint"""
    ...
    strategy = SaveFedMedianModelStrategy(
        fraction_fit = 0.8,
